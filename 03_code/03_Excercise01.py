...
aug_lib = np.zeros(np.shape(tiles))

fig, (series1) = plt.subplots(1,  20, figsize=(13, 3))
k = 0
#aug_lib = np.zeros(np.shape(augmented))
while sample_size >= k:
    augmented, augmented_label = img_aug.flow(tiles[:110], tileLabels[:110], batch_size=11, shuffle=False)
    image_lib = real_aug[0]

    augmented, augmented_label = next(real_aug)
    for series2, aug_img, aug_label in zip(series1, augmented, augmented_label):
        series2.imshow(aug_img[:, :, 0])
